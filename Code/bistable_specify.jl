...
function mat_mul(mat1::Array{Float32,2}, mat2::Array{Float32,2})
    return mat1 .*mat2
end
...
function initial_grid(size,initial_cond,std)::Array{Float32,2}
...
a1 = zeros(Float32,grid_size,grid_size,5) #1=itself; 2,3,4,5=N,S,E,W
a2 = zeros(Float32,grid_size,grid_size,5)
a3 = zeros(Float32,grid_size,grid_size,5)
a4 = zeros(Float32,grid_size,grid_size,5)
...
std = 266*3
a = initial_grid(sizes,[133,133],std)
@time begin
    let grid_run = initial_grid(sizes,[133,133],std)
        ...
        dt_mat = dt .+zeros(Float32,sizes[1],sizes[1])
        Plots.display(heatmap(grid0,title=string(dt*t),c = :cividis))

        while t<t_end/dt
            t+=1
            if t % trunc(Int,(t_end/dt)/10)==0
               print("Process @ ",t,"/",t_end/dt,". Effective t = ",dt*t,"\n")
               print("Total prob = ",sum(grid_run),"\n\n")
               fn = Plots.display(heatmap(grid_run,title=string(dt*t),c = :cividis))
               CSV.write(main_path*"/size=300,center=133,133,std=$std,dt=$dt.csv",  DataFrame(grid_run), writeheader=false)
               savefig(main_path*"/myplot.png")
            end

            #evaluate shifted grid, always NESW syntax
            grid_N = similar(grid_run)
            grid_E = similar(grid_run)
            grid_S = similar(grid_run)
            grid_W = similar(grid_run)
            for j in 1:grid_size
                try
        ...
        increment = similar(grid_run)
        increment += (   mat_mul(a1[:,:,5],grid_W) - mat_mul(a1[:,:,1],grid_run) +
                            mat_mul(a2[:,:,3],grid_E) - mat_mul(a2[:,:,1],grid_run) +
                            mat_mul(a3[:,:,2],grid_N) - mat_mul(a3[:,:,1],grid_run) +
                            mat_mul(a4[:,:,4],grid_S) - mat_mul(a4[:,:,1],grid_run)    )
        grid_run+=mat_mul(dt_mat,increment)
        end
    end
end
